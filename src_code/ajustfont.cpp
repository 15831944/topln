...
void
CAjustFont::chgFont()throw(Acad::ErrorStatus)
{
	int flag = 0;
	for(m_txtStlTblItr->start();!m_txtStlTblItr->done();m_txtStlTblItr->step())
	{
		...
		else
		{
			...
			case SHAPE_TEXT:
				...
				break;
			case WIN_TEXT:
				m_txtStlTblRcd->setFont(m_szFontName.c_str(),0,0,0,0);
				break;
			default:
				acutPrintf(_T("\nPls check here."));
				break;
...
void
CAjustFont::newTxtStlTblItr()throw(Acad::ErrorStatus)
{
	int flag;
